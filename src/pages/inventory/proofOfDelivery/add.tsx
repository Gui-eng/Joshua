...
const tableHeaders = ["id", "Quantity", "Product Name", "Batch #","MFG Date.", "EXP Date", "Remarks"]
...
export default function item({ itemInfo, documentData, clientData, pmrInfo} : InferGetServerSidePropsType<typeof getServerSideProps>) {
    ...
    const clientOptions = makeOptions(clientData, 'deliveredTo', ['companyName'])
    const pmrOptions = makeOptions(pmrInfo, 'pmr', ['code', 'firstName', 'lastName'])
    ...
    function handleDelete(data : any){
     ...
     async function handleOnClick(e : React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        ...
        try {
          const res = await axios.post(`http://${HOSTADDRESS}:${PORT}/api/inventory/proofOfDelivery/add`, rawData)
    ...
    <div className='tw-h-screen tw-w-full'>
      <div className='tw-w-screen tw-flex tw-justify-center'>
        <div className='tw-w-[90%]  tw-flex tw-justify-center tw-bg-sky-600 tw-bg-opacity-30 tw-mt-4 tw-py-8'>
            <div className='tw-w-[90%] tw-rounded-tl-lg tw-rounded-tr-lg'>
             <Form>
              <Form.Field>
                <h1 className='tw-font-bold tw-text-2xl'>Proof Of Delivery</h1>
              </Form.Field>
              ...
              <Form.Group>
              <Form.Field required >
                      <label htmlFor="clientId">Delivered To</label>
                      <Dropdown
              ...
              <Form.Group>
                  ...
                  <Form.Field width={5} disabled={selectedItemId === ''}>
                    ...
                    <Input value={handleUndefined(itemData.quantity)} id='quantity' onChange={(e) => {handleOnChange(e, itemData, setItemData)}} min="0" type="number" label={{content : <Dropdown id="unit" color='blue' options={availableQuantityOptions} onChange={(e, item) => {handleOptionsChange(e, item, itemData, setItemData)}}/>, color : "blue"}} labelPosition='right'/>
      ...
      <div className='tw-w-screen tw-flex tw-flex-col tw-pb-52 tw-items-center'>
          ...
          <div className='tw-w-full tw-flex tw-justify-center tw-pt-4'>
            <div className='tw-w-[90%]'>
                {itemArray.length > 0 ? <Button color='blue' onClick={handleOnClick}>Submit Proof Of Delivery</Button> : null}
          </div>
        </div>
      </div>
    </div>
